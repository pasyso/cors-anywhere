...
function withCORS(headers, request) {
  // console.log('withCORS', request.headers['referer']);
  // headers['access-control-allow-origin'] = '*';
  if (request.headers['origin'])
    headers['access-control-allow-origin'] = request.headers['origin'];
  else if (request.headers['referer']) {
    headers['origin'] = request.headers['referer'];
...
function onProxyResponse(response, req, res) {
  // console.log("onProxyResponse", response);
  var proxy = this;
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    if (!req.headers['origin'] && req.headers['referer']) req.headers['origin'] = req.headers['referer'];
    if (!hasRequiredHeaders(req.headers)) {
      // console.log(400, 'Header required', cors_headers);
